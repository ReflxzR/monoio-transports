...
use crate::connectors::{Connector, pollio::HyperIoWrapper};
...
{
    type Connection = HyperIoWrapper<TlsStream<CN>>;
    type Error = TlsError;
    ...
    async fn connect(&self, key: T) -> Result<Self::Connection, Self::Error> {
        ...
        // Using the HyperIoWrapper, you can wrap tls_stream in MonoioIo as well
        // and give it as a handshake input to hyper client
        Ok(HyperIoWrapper::new(tls_stream))
    }
}